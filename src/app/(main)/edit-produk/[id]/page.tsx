...
import {
  ...
  X, AlertTriangle, Image as ImageIcon, CreditCard, QrCode, Tag, Lightbulb, Save
} from 'lucide-react';
...
export default function EditProductPage() {
  ...
  const [form, setForm] = useState<FormData>({
    ...
    paymentMethod: 'bank', bankName: '', bankAccount: '', qrisUrl: '',
  });
  ...
  const validateStep = (): boolean => {
    ...
    if (step === 2) {
      if (form.conditions.length === 0) { setError('Pilih minimal 1 kondisi barang (misal: Tidak ada minus)'); return false; }
    }
    if (step === 3) {
      if (!form.lat) { setError('Deteksi lokasi wajib dilakukan'); return false; }
      if (!form.address.trim()) { setError('Alamat wajib diisi'); return false; }
    }
    if (step === 4) {
      if (form.paymentMethod === 'bank') {
        if (!form.bankName.trim() || !form.bankAccount.trim()) { setError('Nama bank dan nomor rekening wajib diisi'); return false; }
      } else {
        if (!form.qrisUrl.trim()) { setError('Link/kode QRIS wajib diisi'); return false; }
      }
    }
    return true;
  };
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      // Update profile with bank/QRIS info and set role to seller
      await databases.updateDocument(
        DATABASE_ID,
        PROFILES_ID,
        profile.id,
        {
          role: 'seller',
          bank_name: form.paymentMethod === 'bank' ? form.bankName.trim() : null,
          bank_account: form.paymentMethod === 'bank' ? form.bankAccount.trim() : null,
          qris_url: form.paymentMethod === 'qris' ? form.qrisUrl.trim() : null,
        }
      );
  ...
  return (
    <div className={styles.page}>
      ...
      <div className={styles.header}>
        <h1 className={styles.headerTitle} style={{ display: 'flex', alignItems: 'center' }}><Tag size={20} style={{ marginRight: 8 }} /> Edit Barang</h1>
        ...
        <div className={styles.formBody}>
          <div>
            ...
            <div style={{ display: 'flex', gap: 8, marginBottom: 20 }}>
              <button
                className={`btn btn-sm ${form.paymentMethod === 'bank' ? 'btn-primary' : 'btn-ghost'}`}
              ...
              <button
                className={`btn btn-sm ${form.paymentMethod === 'qris' ? 'btn-primary' : 'btn-ghost'}`}
                onClick={() => setForm(p => ({ ...p, paymentMethod: 'qris' }))}
                style={{ flex: 1, gap: 6 }}
              >
                <QrCode size={14} /> QRIS
              </button>
            ...
            {form.paymentMethod === 'bank' ? (
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
                <div className="form-group">
                  <label className="form-label">Link / ID QRIS</label>
                  <input
                    className="form-input"
      ...
      <div className={styles.actions}>
        ...
        {step < STEPS.length - 1 ? (
          ...
          <button
            ...
            style={{ flex: 2 }}
          >
            {loading ? <span className="spinner" /> : <span style={{ display: 'flex', alignItems: 'center', gap: 6 }}><Save size={16} /> Simpan Perubahan</span>}
          </button>
        )}
      </div>
    </div>
